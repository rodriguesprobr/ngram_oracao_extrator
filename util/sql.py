...
ComunicacaoCientificaNgram = db.Table(
    ...
    db.Column("contador", db.Integer, nullable=False)
)
...
def ngram_por_ano_selecionar(tipo):
    ...
    for ano in anos:
        ...
        if tipo == "ocorrencia":
            ...
            sql += f"sum(case when ano = {ano.ano} then contador end) as '{ano.ano}',"
    if tipo == "ocorrencia":
...
def comunicacao_cientifica_ngram_inserir(comunicacao_cientifica_id, ngram_id):
    ...
    if qtde == 0:
        ...
        ).values(contador=ComunicacaoCientificaNgram.c.contador+1)).lastrowid
    return result
